...
// import services
import { useContractFromMap } from '../contractService';

const ContractListItem = ({ id }) => {
  const contract = useContractFromMap(id);

  if(!contract) return <Skeleton />;
  return (
    <li className="list-none p-2 block">
      <Link to={`/contracts/${contract?._id}`}>{contract?.name}</Link>
    </li>
  )
}
...
const Skeleton = () => {
  return (
    <li className="animate-pulse list-none p-2 block cursor-default select-none">
      <p className='bg-gray-600 text-gray-600 w-fit' href="#"> Contract Name</p>
    </li>
  )
}
// add the skeleton to the component so we can access it in other components (ContractList in this case)
ContractListItem.Skeleton = Skeleton;

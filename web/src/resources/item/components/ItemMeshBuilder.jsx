...
function materialFromConfig(materialCfg = {}, enchantment = "none") {
  ...
  const enchMap = {
    lightning: { emissive: 0x7ec8ff, intensity: 0.1 },
    fire: { emissive: 0xff6b3d, intensity: 0.1 },
    frost: { emissive: 0xaad7ff, intensity: 0.1 },
    poison: { emissive: 0x5ac85a, intensity: 0.1 },
    holy: { emissive: 0xffffaa, intensity: 0.1 },
    cursed: { emissive: 0x6b2d6e, intensity: 0.1 },
  };
  if(enchMap[ench]) {
    ...
    mat.emissiveIntensity = enchMap[ench].intensity
  }
...
function potionFlask({ bodyRadius = 0.18, bodySquash = 1.0, neckRadius = 0.06, neckHeight = 0.12, neckFlare = 0.0, lip = 0.02, foot = 0.0, height = 0.35, stopper = true }) {
  const group = new THREE.Group();
  // Body (ellipsoid): start with sphere and squash/stretch in Y
  const body = new THREE.Mesh(new THREE.SphereGeometry(bodyRadius, 24, 24));
  body.scale.y = Math.max(0.6, bodySquash);
  group.add(body);

  // Neck (taper/flare)
  const topR = neckRadius * (1 + Math.max(0, neckFlare));
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(topR, neckRadius, neckHeight, 16));
  neck.position.y = bodyRadius * body.scale.y + neckHeight / 2;
  group.add(neck);

  // Lip (small thickened rim)
  if(lip > 0) {
    const lipH = Math.min(neckHeight * 0.25, lip * 0.5 + 0.01);
    const lipGeom = new THREE.CylinderGeometry(topR + lip, topR + lip, lipH, 16);
    const lipMesh = new THREE.Mesh(lipGeom);
    lipMesh.position.y = neck.position.y + neckHeight / 2 + lipH / 2;
    group.add(lipMesh);
  }

  // Foot ring
  if(foot > 0) {
    const footGeom = new THREE.TorusGeometry(bodyRadius * 0.75, foot * 0.5, 12, 24);
    const footMesh = new THREE.Mesh(footGeom);
    footMesh.rotation.x = Math.PI / 2;
    footMesh.position.y = -bodyRadius * body.scale.y + foot * 0.25;
    group.add(footMesh);
  }

  // Stopper (non-glass cork) — keep its own material
  if(stopper) {
    const corkH = neckHeight * 0.35;
    const corkTopR = topR * 0.95;
    const cork = new THREE.Mesh(
      new THREE.CylinderGeometry(corkTopR * 0.9, corkTopR * 1.15, corkH, 12),
      new THREE.MeshStandardMaterial({ color: 0x8b5a3c, roughness: 0.95, metalness: 0 })
    );
    const lipOffset = lip > 0 ? Math.min(neckHeight * 0.25, lip * 0.5 + 0.01) : 0;
    cork.position.y = neck.position.y + neckHeight / 2 + lipOffset + corkH / 2 + 0.005;
    cork.userData.keepMaterial = true;
    group.add(cork);
  }
...
function buildArmor(cfg, rng) {
  ...
  const chest = breastplate({
    width: cfg.base?.width ?? 0.9,
    height: cfg.base?.height ?? 1.1,
    depth: cfg.base?.depth ?? 0.35,
    neckWidth: cfg.base?.neckWidth ?? 0.6,
    neckDepth: cfg.base?.neckDepth ?? 0.22,
    armInset: cfg.base?.armInset ?? 0.28,
  }, matMetal);
  group.add(chest);
...
function buildConsumable(cfg, rng) {
  const group = potionFlask({
    ...
    bodySquash: cfg.bottle?.bodySquash ?? 1.0,
    neckHeight: (cfg.base?.height ?? 0.35) * (cfg.bottle?.neckHeight ?? 0.35),
    neckFlare: cfg.bottle?.neckFlare ?? 0.0,
    lip: cfg.bottle?.lip ?? 0.02,
    foot: cfg.bottle?.foot ?? 0.0,
    stopper: cfg.bottle?.stopper ?? true,
  });
  ...
  group.traverse((o) => { if(o.isMesh && !o.userData?.keepMaterial) { o.material = glass; o.renderOrder = 0; } });

  // --- Liquid body: spherical segment (matches bulb), plus flat top cap ---
  const bodyR = cfg.base?.bodyRadius ?? 0.18;
  const squash = cfg.bottle?.bodySquash ?? 1.0;
  const liqA = bodyR * 0.92; // horizontal radius (x/z)
  const liqB = bodyR * squash * 0.92; // vertical radius (y)
  ...
  const yLevel = -liqB + (2 * liqB) * fill * 0.96; // keep under the neck
  ...
  for(let i = 0; i <= samplesY; i++) {
    const t = i / samplesY;
    const y = THREE.MathUtils.lerp(-liqB, yLevel, t);
    const rad = liqA * Math.sqrt(Math.max(0, 1 - (y * y) / (liqB * liqB)));
  ...
  const rTop = liqA * Math.sqrt(Math.max(0, 1 - (yLevel * yLevel) / (liqB * liqB)));
  if(rTop > 1e-4) {
  ...
  if(meniscus > 0 && rTop > 1e-4) {
    ...
    const rOuter = Math.min(rTop * 0.99, rTop + Math.max(meniscus * 6, rTop * 0.08)); const prof = [];
    const steps = 12;
  ...
  if(bubbleCount > 0) {
    ...
    for(let i = 0; i < bubbleCount; i++) {
      const y = THREE.MathUtils.lerp(-liqB * 0.98, yLevel * 0.98, Math.random());
      const radAtY = liqA * Math.sqrt(Math.max(0, 1 - (y * y) / (liqB * liqB)));
...
function WeaponPanel({ overrides, setOverrides }) {
  ...
  return (
    <div className="absolute top-3 right-3 z-[999] bg-black/60 text-white rounded-xl px-3 py-2 w-64 space-y-2">
      <div className="text-xs font-semibold opacity-90">Weapon Controls</div>
      <label className="flex items-center justify-between text-xs">Enchantment
        <input type="checkbox" className="ml-2" checked={overrides.enchantmentEnabled} onChange={(e) => setOverrides(o => ({ ...o, enchantmentEnabled: e.target.checked }))} />
      </label>
...
function TrinketPanel({ overrides, setOverrides }) {
  ...
  return (
    <div className="absolute top-3 right-3 z-[999] bg-black/60 text-white rounded-xl px-3 py-2 w-48 space-y-2">
      <div className="text-xs font-semibold opacity-90">Trinket Controls</div>
      <label className="flex items-center justify-between text-xs">Enchantment
        <input type="checkbox" className="ml-2" checked={overrides.enchantmentEnabled} onChange={(e) => setOverrides(o => ({ ...o, enchantmentEnabled: e.target.checked }))} />
      </label>
...
function ConsumablePanel({ overrides, setOverrides }) {
  ...
  const setBottle = (k, v) => set("bottle", { ...c.bottle, [k]: v });
  return (
    <div className="absolute top-3 right-3 z-[999] bg-black/60 text-white rounded-xl px-3 py-2 w-80 space-y-2">
      <div className="text-xs font-semibold opacity-90">Consumable Controls</div>
      <label className="flex items-center justify-between text-xs">Enchantment
        <input type="checkbox" className="ml-2" checked={overrides.enchantmentEnabled} onChange={(e) => setOverrides(o => ({ ...o, enchantmentEnabled: e.target.checked }))} />
      </label>
      <div className="text-xs font-semibold opacity-90">Potion / Consumable</div>
      <div className="text-[10px] uppercase opacity-70">Bottle</div>
      <label className="block text-xs">Body squash: {c.bottle.bodySquash.toFixed(2)}x
        <input type="range" min={0.7} max={1.4} step={0.01} value={c.bottle.bodySquash} onChange={(e) => setBottle("bodySquash", Number(e.target.value))} className="w-full" />
      </label>
      <label className="block text-xs">Neck height: {(c.bottle.neckHeight * 100).toFixed(0)}%
        <input type="range" min={0.15} max={0.60} step={0.01} value={c.bottle.neckHeight} onChange={(e) => setBottle("neckHeight", Number(e.target.value))} className="w-full" />
      </label>
      <label className="block text-xs">Neck flare: {c.bottle.neckFlare.toFixed(2)}
        <input type="range" min={0} max={0.6} step={0.01} value={c.bottle.neckFlare} onChange={(e) => setBottle("neckFlare", Number(e.target.value))} className="w-full" />
      </label>
      <label className="block text-xs">Lip size: {c.bottle.lip.toFixed(3)}
        <input type="range" min={0} max={0.12} step={0.005} value={c.bottle.lip} onChange={(e) => setBottle("lip", Number(e.target.value))} className="w-full" />
      </label>
      <label className="block text-xs">Foot size: {c.bottle.foot.toFixed(3)}
        <input type="range" min={0} max={0.12} step={0.005} value={c.bottle.foot} onChange={(e) => setBottle("foot", Number(e.target.value))} className="w-full" />
      </label>
      <label className="flex items-center justify-between text-xs">Stopper
        <input type="checkbox" className="ml-2" checked={!!c.bottle.stopper} onChange={(e) => setBottle("stopper", e.target.checked)} />
      </label>
...
export default function MeshBuilderDemo() {
  ...
  const [overrides, setOverrides] = React.useState({
    ...
    consumable: { glassTint: baseConfig.material?.tint || "#aaddff", glassOpacity: 0.35, liquidTint: baseConfig.liquidTint || "#55aaff", liquidOpacity: 0.7, fill: 0.75, meniscus: 0.0, bubbles: 0, bottle: { bodySquash: 1.0, neckHeight: 0.35, neckFlare: 0.0, lip: 0.02, foot: 0.0, stopper: true } },
  ...
  const mergedConfig = React.useMemo(() => {
    ...
    if(c.type === "consumable") {
      Object.assign(c, {
        ...
        bottle: overrides.consumable.bottle,
      });
    }

 ...
 * @param {string?} props.displayKey - the key to use for the display value if options is an array of objects, do not set if options is an array of strings
 ...
 * @param {(string[]|object[])} props.options - the options to display in the list, can be an array of strings or objects
 ...
 * @param {string?} props.valueKey - the key to use for the value if options is an array of objects, do not set if options is an array of strings
 */
const SelectInput = ({
  autoFocus = false
  , change
  ...
  if(handleFilter) filterable = true; // if a handleFilter function is passed, we should assume the user wants to filter the options
  if(name === 'tier') {
  ...
  // always set the filtered options to the options prop when it changes (to support mutation of the options array from outside the component)
  useEffect(() => {
  ...
  if(options?.[0] && typeof options?.[0] === 'object' && (!displayKey || !valueKey)) {
    console.error(`Error in SelectInput for name: ${name}: if options is an array of objects, you must also pass displayKey and valueKey`);
    return null;
  }
  if(options?.[0] && typeof options?.[0] === 'string' && (displayKey || valueKey)) {
    console.warn(`Error in SelectInput for name: ${name}: if options is an array of strings, you shouldn't pass a displayKey or valueKey`);
  }

  // get the item type so we know how to handle it
  const itemType = typeof options?.[0]

  // default search filter if no onSearch is passed, will filter the options array by the displayKey (or the item itself if it's a string)
  const filterOptions = (searchString) => {
    if((!searchString || !filteredOptions) && options?.length) {
      setFilteredOptions(options);
    }
    if(options?.[0] && typeof options?.[0] === 'object') {
      setFilteredOptions(options.filter(item => item[displayKey].toLowerCase().includes(searchString.toLowerCase())));
    }
    if(options?.[0] && typeof options?.[0] === 'string') {
      setFilteredOptions(options.filter(item => item.toLowerCase().includes(searchString.toLowerCase())));
    }
  }

  // if handleFilter is passed, use that, otherwise use the default filter
  const setFilter = handleFilter || filterOptions;

  const getSelectedItem = () => {
    if(!selectedValue) return null;
    if(itemType === 'string') {
      // return a string containing each selected item separated by a comma
      if(multiple) return options?.filter(item => item && selectedValue.includes(item));
      return selectedValue;
    } else {
      if(multiple) return options?.filter(item => item && selectedValue.includes(item[valueKey]));
      return options?.find(item => item && item[valueKey] == selectedValue);
    }
  }
  ...
  return (
    <Combobox
      as="div"
      className={`relative w-full max-w-xs border-transparent z-20 text-left bg-black text-white ${className}`}
      disabled={disabled}
    ...
    >
      {label ? (
        <Combobox.Label
          htmlFor={name}
          className="px-2 pt-1 text-xs absolute top-0 text-white bg-transparent z-10"
        >
      ...
      <div className="relative">
        {filterable ? (
          <Combobox.Input
            autoFocus={autoFocus}
            name={name}
            className={`px-2 min-h-[44px] text-sm text-white text-left truncate ${label ? 'pt-5 pb-1' : 'pt-3 pb-2'} border-none block w-11/12 mt-0 disabled:opacity-50`}
        ...
        <Combobox.Button
          className={`absolute top-0 bottom-0 px-2 text-sm text-white text-left overflow-x-hidden ${label ? 'pt-5 pb-1' : 'pt-3 pb-2'} block w-full mt-0 border border-gray-300 rounded focus:outline-none focus:ring-0 focus:border-gray-400 disabled:opacity-50`}
        >
          {({ open }) => (
            <>
                    ...
                    {({ active, selected }) => (
                      multiple ? (
                        <>
                          ...
                          {selected && (
                            <span
                              className='absolute inset-y-0 left-0 flex options-center pl-1.5 text-indigo-600'
                            >
